...
#======================================
prob = knn_model.predict_proba(Xtrain2)
new_set = init_set(prob)
new_set = update_set(new_set, prob)

prob = lda_model.predict_proba(Xtrain2)
new_set = update_set(new_set, prob)

prob = nn_model.predict_proba(Xtrain2)
new_set = update_set(new_set, prob)

prob = rf_model.predict_proba(Xtrain2) 
new_set = update_set(new_set, prob)

prob = svm_model.predict_proba(Xtrain2) 
new_set = update_set(new_set, prob)

prob = xgb_model.predict_proba(Xtrain2)
new_set = update_set(new_set, prob)
...
forest = RandomForestClassifier(n_estimators=50, n_jobs=-1, max_features='sqrt', min_samples_leaf=1, class_weight='balanced')
forestCal2 = CalibratedClassifierCV(forest, method='isotonic', cv=6)
forestCal2 = forestCal2.fit(Xtrain2, ytrain2)
...
print '\nRF: Logloss (with calibration using isotonic) = ' + str(logloss2) + ' compare:', str(logloss)
